...
def main(mydata):
    ...
    while True:
        ...
        for i in label.keys():
            ...
            update(label,data)
        for (node,tag) in label.items():
            history[node].append(tag)
    return history
def calcFinalTag(label,r):
    #这里r代表预先设置的阈值，超过该阈值表示与一个tag相关联
    finalTag = {}
    for node in label.keys():
        counter = collections.Counter(label[node])
        finalTag[node]=[x[0] for x in counter.items() if (x[0]>=r)]
    return finalTag
...
if __name__== '__main__':
    ...
    for filename in edgesFiles:
        ...
        print(calcFinalTag(main(graph),3))
